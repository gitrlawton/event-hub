...
export function WhosGoing({ event, currentUserId, compact = false }: WhosGoingProps) {
  ...
  const followingAttendees = useMemo(() => {
    ...
    const attendingFollowingIds = event.attendeeIds.filter(id => followingIds.includes(id));
    
    return attendingFollowingIds.map(id => getUserById(id)).filter(Boolean) as User[];
